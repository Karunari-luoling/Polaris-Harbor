...
export type RepoPlatform = 'github' | 'gitlab' | 'gitee' | 'gitea' | 'gitcode' | 'unknown';

const normalizeHost = (input: string) => input.replace(/^www\./i, '').toLowerCase();

export const detectRepoPlatform = (repoUrl?: string): RepoPlatform => {
  ...
  try {
    ...
    if (host === 'gitea.com' || host.includes('gitea')) return 'gitea';
    if (host.includes('gitcode')) return 'gitcode';

    return 'unknown';
  } catch {
    // If repoUrl is not a valid absolute URL, fall back to string matching.
    const raw = repoUrl.toLowerCase();
    if (raw.includes('github')) return 'github';
    if (raw.includes('gitlab')) return 'gitlab';
    if (raw.includes('gitee')) return 'gitee';
    if (raw.includes('gitea')) return 'gitea';
    if (raw.includes('gitcode')) return 'gitcode';
    return 'unknown';
  }
};

const platformTitle: Record<RepoPlatform, string> = {
  github: 'GitHub',
  gitlab: 'GitLab',
  gitee: 'Gitee',
  gitea: 'Gitea',
...
// Simple Icons paths (viewBox 0 0 24 24). We inline to avoid extra assets/deps.
const iconPath: Record<Exclude<RepoPlatform, 'unknown'>, string> = {
  github:
...
export const getRepoPlatformLabel = (repoUrl?: string) => platformTitle[detectRepoPlatform(repoUrl)];
